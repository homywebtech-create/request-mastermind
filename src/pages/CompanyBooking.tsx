...
export default function CompanyBooking() {
  ...
  const isTimeSlotAvailable = (timeSlot: string, selectedDate: Date | null, specialistId?: string) => {
    ...
    // CRITICAL FIX: Use UTC to match database times
    const slotStartDateTime = new Date(selectedDate);
    slotStartDateTime.setUTCHours(hours, minutes, 0, 0);
    ...
    if (specialistId && specialistSchedules[specialistId]) {
      ...
      console.log(`✅ وجدنا ${schedules.length} حجز للمحترف`);
      ...
      console.log(`📅 الحجز الجديد (${hoursCount} ساعة):`);
      console.log(`  UTC: ${slotStartDateTime.toISOString()} إلى ${slotEndDateTime.toISOString()}`);
      ...
      const hasConflict = schedules.some((schedule, index) => {
        ...
        console.log(`  UTC من DB: ${scheduleStart.toISOString()} إلى ${scheduleEnd.toISOString()}`);
        
        const travelBufferMinutes = schedule.travel_buffer_minutes || 120;
        ...
        console.log(`  المنطقة المحمية (2س قبل + الحجز + 2س بعد):`);
        console.log(`    ${existingBookingStartWithBuffer.toISOString()} إلى ${existingBookingEndWithBuffer.toISOString()}`);
        ...
        console.log(`  النتيجة: ${overlaps ? '❌ محجوز' : '✅ متاح'}`);
